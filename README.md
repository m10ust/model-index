...
**Deploying.** `index.html` references `styles.css` and `app.js` with a `?v=` stamp. Bump it on every push that changes either file, otherwise a browser that has the new HTML can still run the previous script against it, which shows up as a control that silently does nothing. The dataset fetch already sends `cache: no-store` so the data cannot go stale the same way.

**Cost axis.** The dashboard also reports value: the fleet score divided by the blended price per million tokens, which answers a different question from capability. Blended pricing is Artificial Analysis's 7:2:1 cache-hit/input/output ratio. The cache-hit rate is broken out separately because a cache-heavy agent workload pays mostly at that rate, and it is the number that actually governs the cost of running this fleet.
